...
public class SeataInitiator implements SmartLifecycle {
    ...
    private void doStart() {
        ...
        if(StringUtils.isNotBlank( properties.getEnv())){
            System.setProperty(ConfigurationFactory.ENV_PROPERTY_KEY, properties.getEnv());
        }

        MetricsManager.get().init();

        ThreadPoolExecutor messageExecutor =
                new ThreadPoolExecutor(properties.getMinServerPoolSize(), properties.getMaxServerPoolSize(), properties.getKeepAliveTime(),
                        TimeUnit.SECONDS, new LinkedBlockingQueue<>(properties.getMaxTaskQueueSize()),
